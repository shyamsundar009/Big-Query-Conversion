CREATE OR REPLACE PROCEDURE NYU_V_ED_SCPM ( numDays in integer) is
-- Few changes:
...
-- CREATE GLOBAL TEMPORARY TABLE NYUGT_ED_TMP ON COMMIT PRESERVE ROWS AS SELECT * FROM ED_TMP WHERE ROWNUM < 0;
-- CREATE GLOBAL TEMPORARY TABLE NYUGT_ED_ORD_TMP ON COMMIT PRESERVE ROWS AS SELECT * FROM ED_ORD_TMP WHERE ROWNUM < 0;
-- CREATE GLOBAL TEMPORARY TABLE NYUGT_ED_ADT_TMP ON COMMIT PRESERVE ROWS AS SELECT * FROM ED_ADT_TMP WHERE ROWNUM < 0;
-- create index X1_NYUGT_ED_ADT on NYUGT_ED_ADT_TMP(EVENT_ID);
-- create index X2_NYUGT_ED_ADT on NYUGT_ED_ADT_TMP(PAT_ENC_CSN_ID);
...
clarity.NYU_CLARITY_LOG_PKG.logMsg( 'NYU_V_ED_SCPM', 'NYU_V_ED_SCPM_J', 'INFO', 'START', 'STORED PROCEDURE STARTED');
...
/*
Date: 7/19/2023
Developer Name: Cecilia Yang
Modification: Added more LEV values to capture ED Departure timestamps
*/
,'222'  --ED DISPOSITION SELECTED
,'160000777'  --NYU ERS PATIENT LEFT ED
,'49000'  --ED PATIENT ERROR
              )
...
LEFT JOIN
(SELECT PSH.*
,ROW_NUMBER() OVER (PARTITION BY PSH.PAT_ENC_CSN_ID ORDER BY PSH.UPDATE_TIME DESC NULLS LAST) AS PSH_RNK
FROM
(SELECT DISTINCT
PESH.PAT_ENC_CSN_ID,
PESH.UPDATE_TIME,
PESH.UPDATED_CONF_STAT_C

FROM PAT_ENC_STAT_HX PESH
) PSH
) PESH ON PESH.PAT_ENC_CSN_ID=EEI.PAT_ENC_CSN_ID AND PESH.PSH_RNK=1  --LAST STATUS
...
where PATIENT_ARRIVED_IN_ED IS NOT NULL --09/28/2022 CY per email From: Chakrabarti, Samita Sent: Thursday, September 22, 2022 1:31 PM
;
...
clarity.NYU_CLARITY_LOG_PKG.logMsg( 'NYU_V_ED_SCPM', 'ED_TMP', 'INFO', 'DONE', 'INSERT INTO ... SELECT FROM ... ');
...
;
COMMIT;
clarity.NYU_CLARITY_LOG_PKG.logMsg( 'NYU_V_ED_SCPM', 'ED_ORD_TMP', 'INFO', 'DONE', 'INSERT INTO ... SELECT FROM ... ');
...
commit;
clarity.NYU_CLARITY_LOG_PKG.logMsg( 'NYU_V_ED_SCPM', 'ED_ADT_TMP', 'INFO', 'DONE', 'INSERT INTO ... SELECT FROM ... ');

-- execute immediate 'create unique index IDX_ED_ADT_U on ED_ADT_TMP (EVENT_ID)';visit_date_time
-- execute immediate 'create index IDX_ED_ADT on ED_ADT_TMP (PAT_ENC_CSN_ID)';

execute immediate 'Truncate table SCPM_ED';
...
*/
insert /*+ APPEND PARALLEL(8) ENABLE_PARALLEL_DML */  into SCPM_ED
with FLO as (
  select * from
...
--    TRUNC ((TO_NUMBER (TO_CHAR (HAR.ADM_DATE_TIME, 'YYYYMMDD')) - TO_NUMBER (TO_CHAR (P.BIRTH_DATE, 'YYYYMMDD'))) / 10000) AS "PATIENT_AGE", --Added 01/30/12 as per Jill Pinnella PMO
 --YC 7/20/2017 SR#2360378
         case when months_between(trunc( nvl(HAR.ADM_DATE_TIME ,peh.hosp_admsn_time)),trunc(p.BIRTH_DATE))<0  then 0 -- YC 8/3/2017 to avoid situation when NB admission is prior to BD
          when floor(months_between( nvl(HAR.ADM_DATE_TIME ,peh.hosp_admsn_time),p.BIRTH_DATE)/12)=0 then
            round(months_between( nvl(HAR.ADM_DATE_TIME ,peh.hosp_admsn_time),p.BIRTH_DATE)/12,2)
            else floor(months_between( nvl(HAR.ADM_DATE_TIME ,peh.hosp_admsn_time),p.BIRTH_DATE)/12) end AS "PATIENT_AGE",
    ...
    CASE
                 WHEN HAR.COVERAGE_ID IS NULL
                 THEN
                    ZCFC2.NAME
                 WHEN HAR.COVERAGE_ID IS NOT NULL
                 THEN
                    CASE
...
--    EPP.PRODUCT_TYPE      AS "INSURANCE_PRODUCT",
   ...
   /* CASE
      WHEN  nvl(HAR.ADM_DATE_TIME ,peh.hosp_admsn_time)>=to_date('11/09/2012','mm/dd/yyyy')
      THEN*/ ZCED.NAME
      /*ELSE DECODE(ZCED.ED_DISPOSITION_C,'3','Admit','4','AMA','8','Expired','9','Send to L&D','11','Transfer to Procedure Area',ZCED.NAME)
    END*/ AS "ED_DISCH_DISPOSITION",
   /* CASE
      WHEN  nvl(HAR.ADM_DATE_TIME ,peh.hosp_admsn_time)>=to_date('11/09/2012','mm/dd/yyyy')
      THEN*/ ZCED.ABBR
     /* ELSE DECODE(ZCED.ED_DISPOSITION_C,'3','Admit','4','AMA','8','Deceased','9','Send to L&D','11','Transf-Proc',ZCED.ABBR)
    ...
    nvl(ZCPATCLS.NAME,zpc.name)      AS "ENC_FINAL_PAT_CLASS",--Added 01/30/12 as per PMO -- 7/28/2021 YC SR#3066359
...
/*******************************/
    TRIAGE_STARTED,
    IP_BED_REQUESTED,
--   COALESCE(PATIENT_DEPARTED_FROM_ED,PATIENT_ADMITTED,PATIENT_LEFT_ED,ED_DISPOSITION_SELECTED) AS PATIENT_DEPARTED_FROM_ED,
/*
Date: 11/1/2023
Developer Name: Cecilia Yang
Modification: Updated PER EMAIL FROM JILLIAN Sent: Wednesday, November 1, 2023 3:35 PM
Current "ED Departure" Logic
1.	If LEV 95 exists, use the event time for LEV 95, else LEV 49000 (if LEV 49000 >=ED arrival date time) ? LEV 49000 populates if LEV 95 errors out (populates a system error); LEV 49000 only exists if LEV 95 doesn?t
2.	If LEV 95 is missing, look at LEV 65, if that exists and the ADT event id (IEV 100) also exists, use the event time for LEV 65  (Admitted from ED) - remove
3.	If LEV 65 is missing, look at LEV 160000777, if that exists, use the event time for LEV 160000777 (NYU ERS PATIENT LEFT ED) - remove
4.	If LEV 160000777 is missing, look at LEV 222, if that exists, use the event time for LEV 222  (ED DISPOSITION SELECTED) - remove
*/
(CASE WHEN PATIENT_DEPARTED_FROM_ED IS NOT NULL THEN PATIENT_DEPARTED_FROM_ED
  WHEN PATIENT_DEPARTED_FROM_ED IS NULL AND ED_PATIENT_ERROR>=PATIENT_ARRIVED_IN_ED THEN ED_PATIENT_ERROR END) AS PATIENT_DEPARTED_FROM_ED,
...
/*******************************/
 ...
 --   ord5.order_time       AS DISCH_order_date
    ...
    ,CASE
                 ...
                 THEN
                    'HJD-P'
                 WHEN (xtr.LOC_ID NOT IN (10530, 10500, 10510,10800,1084001,1086001,1086) ---YC 5/8/2019 ) -- YC 3/21/2016 added Lutheran
...
,    /*CASE
      WHEN  nvl(HAR.ADM_DATE_TIME ,peh.hosp_admsn_time)>=to_date('11/09/2012','mm/dd/yyyy')
      THEN*/ ZCED.ED_DISPOSITION_C
     /* ELSE DECODE(ZCED.ED_DISPOSITION_C,'3','4','4','9','8','12','9','11','11','8',ZCED.ED_DISPOSITION_C)
    END AS ED_DISPOSITION_C*/
,nvl(peh.adt_pat_class_c,har.acct_class_ha_c) adt_pat_class_c
...
--7/23/21 YC to synch with DISCHARGE_X  LEFT OUTER JOIN CLARITY_EPM EPM  ON HAR.PRIMARY_PAYOR_ID = EPM.PAYOR_ID
  LEFT JOIN COVERAGE CVG    ON HAR.COVERAGE_ID = CVG.COVERAGE_ID
  LEFT OUTER JOIN clarity_epm EPM ON cvg.payor_id = EPM.PAYOR_ID
--7/23/21 YC to synch with DISCHARGE_X  LEFT OUTER JOIN CLARITY_EPP EPP  ON HAR.PRIMARY_PLAN_ID = EPP.BENEFIT_PLAN_ID
        ...
        from
          ...
          -- FROM HSP_ACCT_INS_BKTS HAR1
           -- where HAR1.LINE =
            ...
            -- join NYUGT_ED_TMP t on t.hsp_account_id=HAR1X.hsp_account_id -- YC 1/28/2019 filter by tmp - to speed up
  ...
  LEFT OUTER JOIN
   ...
   -- join NYUGT_ED_TMP t on t.pat_enc_csn_id=ADT2.PAT_ENC_CSN_ID -- YC 1/28/2019 filter by tmp - to speed up
  ...
  LEFT OUTER JOIN
   ...
   -- join NYUGT_ED_TMP t on t.pat_enc_csn_id=ADT6.PAT_ENC_CSN_ID -- YC 1/28/2019 filter by tmp - to speed up
...
--  LEFT OUTER JOIN CLARITY_EDG CE_ALT  ON altdx.fin_dx_alt_id = CE_ALT.DX_ID
  ...
  left join   ZC_PAT_CLASS zpc on zpc.adt_pat_class_c=har.ACCT_CLASS_HA_C -- 7/28/21 YC SR#3066359 -- to use if peh.ADT_PAT_CLASS_C is null
...
/*  LEFT OUTER JOIN
    (SELECT max(pp.description) description, --- YC 10/30/2015 make it max - there could be both orders at the same time
        ...
        join NYUGT_ED_TMP t on t.pat_enc_csn_id=pp.PAT_ENC_CSN_ID -- YC 1/28/2019 filter by tmp - to speed up
  ...
  LEFT OUTER JOIN
    ...
    join NYUGT_ED_TMP t on t.pat_enc_csn_id=pppp.PAT_ENC_CSN_ID -- YC 1/28/2019 filter by tmp - to speed up
  ...
  LEFT OUTER JOIN ( ---YC 10/30 2015 take the max , since there still could be several
       select ord5.pat_enc_csn_id, max(order_time) order_time
              ...
              join NYUGT_ED_TMP t on t.pat_enc_csn_id=ord5.PAT_ENC_CSN_ID -- YC 1/28/2019 filter by tmp - to speed up
 ...
 */
  LEFT OUTER JOIN
    ...
    WHERE adt0.effective_time IN
      ...
      --join NYUGT_ED_TMP t on t.pat_enc_csn_id=adt5.PAT_ENC_CSN_ID -- YC 1/28/2019 filter by tmp - to speed up
...
Date: 4/18/2023
Developer Name: Cecilia Yang
Modification: Added secondary insurance info
*/

LEFT OUTER JOIN HSP_ACCT_CVG_LIST CVG_LIST
    ON HAR.HSP_ACCOUNT_ID=CVG_LIST.HSP_ACCOUNT_ID  and CVG_LIST.LINE=2
...
AND PATIENT_ARRIVED_IN_ED IS NOT NULL --09/28/2022 CY per email From: Chakrabarti, Samita Sent: Thursday, September 22, 2022 1:31 PM

--to only include LICH patients from 11/6/22 12:00AM, PER SAMITA'S EMAIL Sent: Monday, November 7, 2022 12:40 PM
AND ((xtr.parent_loc_id NOT IN (1086001,1086) AND EAF.LOC_ID NOT IN (1086001,1086))
OR ((xtr.parent_loc_id  IN (1086001,1086) or EAF.LOC_ID IN (1086001,1086)) AND trunc(nvl(HAR.ADM_DATE_TIME ,peh.hosp_admsn_time)) >= TO_DATE('11/06/2022','MM/DD/YYYY')))

AND CANCELED_ADM IS NULL
  ;
  commit;
clarity.NYU_CLARITY_LOG_PKG.logMsg( 'NYU_V_ED_SCPM', 'SCPM_ED', 'INFO', 'DONE', 'INSERT INTO ... SELECT FROM ... ');

---   end LOOP;
--execute immediate 'DROP TABLE ED_TMP PURGE';
--execute immediate 'DROP TABLE ED_ORD_TMP PURGE';

clarity.NYU_CLARITY_LOG_PKG.logMsg( 'NYU_V_ED_SCPM', 'NYU_V_ED_SCPM_J', 'INFO', 'COMPLETED', 'STORED PROCEDURE COMPLETED');
