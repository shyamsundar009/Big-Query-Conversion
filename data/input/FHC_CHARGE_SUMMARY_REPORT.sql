CREATE OR REPLACE PROCEDURE FHC_CHARGE_SUMMARY_REPORT
(
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE FHC_CHARGE_SUMMARY_POPULATION_GTT';
  --DROP TABLE FHC_CHARGE_SUMMARY_POPULATION_GTT;
  --CREATE GLOBAL TEMPORARY TABLE FHC_CHARGE_SUMMARY_POPULATION_GTT AS 
  INSERT INTO FHC_CHARGE_SUMMARY_POPULATION_GTT
  WITH REPORT_PARAM AS (
    SELECT -- SP-SQL UUID: 7f97f9c3-2be6-4777-9557-5e027a65e80f
    EPIC_UTIL.EFN_DIN(BEGIN_DATE) START_DATE, EPIC_UTIL.EFN_DIN(END_DATE) END_DATE
    FROM DUAL
  )
  ,POPULATION AS (
    ...
    ,ser.PROV_TYPE PROV_PRIM_SPEC
    ,ENC_TYPE.name ENC_TYPE
    ,ZAS.NAME as APPT_STATUS_NAME
    ,(CASE WHEN PT.PAT_MRN_ID IS NULL THEN '*No Patient Record for #' || TO_CHAR(PT.PAT_ID) || ' *'
    ...
    ,(CASE WHEN PE.VISIT_PROV_ID IS NULL THEN '*No Visit Provider*'
        ...
        ELSE SER.PROV_NAME || ' [' || SER.PROV_ID || ']' END) as VISIT_PROVIDER
    ,DEP.DEPARTMENT_ID
    ,DEP.DEPARTMENT_NAME
    ,center.name DEPARTMENT_CENTER
    ,LOC.LOC_ID
    ,LOC.LOC_NAME
    ,PRC.PRC_NAME
  ...
  EXECUTE IMMEDIATE 'TRUNCATE TABLE FHC_CHARGE_SUMMARY_BILLING_GTT';
  --DROP TABLE FHC_CHARGE_SUMMARY_BILLING_GTT;
  --CREATE GLOBAL TEMPORARY TABLE FHC_CHARGE_SUMMARY_BILLING_GTT AS 
  ...
  WITH BILLING_HTR_CODES AS(
    ...
    FROM (
      ...
      --,POPULATION.PAT_BIRTH_DATE
      --,HSP_TRANSACTIONS.TX_ID
      ,HSP_TRANSACTIONS.PLACE_OF_SVC_ID CODE_POS_ID
      ,HSP_TRANSACTIONS.HCPCS_CODE
      ,HSP_TRANSACTIONS.CPT_CODE
      ,HSP_TRANSACTIONS.MODIFIERS CODE_MODIFIERS
      ,'CPT' CODE_TYPE
      ,SERVICE_DATE CODE_SERVICE_DATE
      ,'HTR' CODE_SOURCE
      ,HSP_TRANSACTIONS.PROC_ID CODE_ID
      ,HSP_TRANSACTIONS.CHG_ROUTER_SRC_ID CODE_UCL_ID
  ...
  ,BILLING_TDL_CODES AS (
    SELECT /*+ INDEX(ARPB_TRANSACTIONS EIX_PB_TRANSACTIONS_PATENC_CSN)*/ POPULATION.PAT_ID
    ,POPULATION.PAT_ENC_DATE_REAL
    ,POPULATION.PAT_ENC_CSN_ID
    --,POPULATION.PAT_BIRTH_DATE
    --,CLARITY_TDL_TRAN.TX_ID
    ,ARPB_TRANSACTIONS.SERVICE_AREA_ID CODE_POS_ID
    ,CLARITY_TDL_TRAN.CPT_CODE CODE
    ,CLARITY_TDL_TRAN.MODIFIER_ONE ||
      CASE WHEN CLARITY_TDL_TRAN.MODIFIER_TWO IS NOT NULL THEN ',' || CLARITY_TDL_TRAN.MODIFIER_TWO END ||
      CASE WHEN CLARITY_TDL_TRAN.MODIFIER_THREE IS NOT NULL THEN ',' || CLARITY_TDL_TRAN.MODIFIER_THREE END ||
      CASE WHEN CLARITY_TDL_TRAN.MODIFIER_FOUR IS NOT NULL THEN ',' || CLARITY_TDL_TRAN.MODIFIER_FOUR END
    CODE_MODIFIERS
    ,'CPT' CODE_TYPE
    ,ARPB_TRANSACTIONS.SERVICE_DATE CODE_SERVICE_DATE
    ,'ETR' CODE_SOURCE
    ,CLARITY_TDL_TRAN.PROC_ID CODE_ID
    ,ARPB_TRANSACTIONS.CHG_ROUTER_SRC_ID CODE_UCL_ID
    ,PRE_AR_CHG.AMOUNT TX_AMOUNT
    FROM FHC_CHARGE_SUMMARY_POPULATION_GTT POPULATION
  ...
  ,UNIFIED_BILLING AS (
    SELECT BILLING_PB_HB.*
    ,CLARITY_POS.POS_ID
    ,CLARITY_POS.POS_CODE
    ,CLARITY_POS.POS_NAME
    ,CLARITY_EAP.PROC_NAME CODE_DESCRIPTION
    ,CLARITY_UCL.CHARGE_SOURCE_C UCL_CHARGE_SOURCE_C
    ,ZC_CHG_SOURCE_UCL.NAME UCL_CHARGE_SOURCE
    FROM (
