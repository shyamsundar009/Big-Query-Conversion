...
CREATE PROCEDURE SP_CIN_CIGNA_DAILY_MATCHED
AS
BEGIN
    INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'starting refresh');
    COMMIT;
    ...
    -- CIGNA COMMERICAL TRANSITION
            SELECT DISTINCT CASE WHEN DAILY.FACILITY LIKE '%NYU%' THEN 'NYU/HJD' ELSE 'OTHER' END AS NYU_OTHER,
            DAILY.PAT_MRN_ID,
            DAILY.PAT_ID,
            ...
            DAILY.GENDER,
            CASE WHEN PAT.BIRTH_DATE IS NOT NULL THEN CONVERT(VARCHAR(10), PAT.BIRTH_DATE, 101) ELSE DAILY.DOB END AS DOB,
            DAILY.CIGNA_ID,
            ...
            CAST(NULL AS VARCHAR(255)) AS TREATMT_SETTING_SUBTYPE, --THESE 4 ADDED BY GLENN FOR EXTRA UHC FIELDS
            CAST(NULL AS VARCHAR(255)) AS RST_SCORE,
            CAST(NULL AS VARCHAR(255)) AS DISP_CODE_DESC,
            CAST(NULL AS VARCHAR(255)) AS RPM_SCORE,
      ...
      INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'ending CIN_CIGNA_DAILY_MV_W_PATID');
            COMMIT;

            INSERT INTO CIN_CIGNA_DAILY_MATCHED
            --UHC INFO
            SELECT DISTINCT CASE WHEN DAILY.HOSP_NAME LIKE '%NYU%' THEN 'NYU/HJD' ELSE 'OTHER' END AS NYU_OTHER,
            DAILY.PAT_MRN_ID,
            DAILY.PAT_ID,
            COALESCE(DAILY.ACTION_DATE, DAILY.DATE_ADDED) AS DATE_ADDED, --wanted date patient was added to the report since they remain on the file and keep getting updated, gl 7/24/14
            ...
            DAILY.GENDER,
            CASE WHEN PAT.BIRTH_DATE IS NOT NULL THEN CONVERT(VARCHAR(10), PAT.BIRTH_DATE, 101) ELSE DAILY.BIRTHDATE END,
            DAILY.ALT_ID,
            DAILY.PHYS_PCP_NAME,
            NULL,--POD_NAME
            ...
            COALESCE(LOB, 'Commercial'), --AETNA LINE_OF_BUSINESS, new data added by UHC 4/1/2016
            NULL, --AETNA PRODUCT,
            ...
            JOIN PATIENT PAT ON DAILY.PAT_ID = PAT.PAT_ID;
      INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'ending CIN_UHC_DAILY_MV_W_PATID');
            COMMIT;
            ...
            DAILY.PAT_MRN_ID,
            DAILY.PAT_ID,
            COALESCE(DAILY.ACTION_DATE, DAILY.DATE_ADDED) AS DATE_ADDED,
            UPPER(CASE WHEN PAT.PAT_LAST_NAME IS NOT NULL THEN PAT.PAT_LAST_NAME ELSE DAILY.PATIENT_LAST_NAME END),
            UPPER(CASE WHEN PAT.PAT_FIRST_NAME IS NOT NULL THEN PAT.PAT_FIRST_NAME ELSE DAILY.PATIENT_FIRST_NAME END),
            DAILY.GENDER,
            CASE WHEN PAT.BIRTH_DATE IS NOT NULL THEN PAT.BIRTH_DATE ELSE CONVERT(DATE, DAILY.BIRTHDATE, 120) END, -- SELECT * FROM CIN_OHP_DAILY_MV_W_PATID  
            ...
            NULL, --AETNA AOS_SCORE,
            NULL, --AETNA MEMBER_PHONE,
            COALESCE(LOB, 'Commercial'), --'Commercial', --AETNA LINE_OF_BUSINESS,
            NULL, --AETNA PRODUCT,
            NULL, --AETNA DEDICATED_UNIT_NAME,
            NULL, --AETNA STAY_TYPE,
            NULL, --AETNA_CARD_ID,
 ...
 READMISSION_RISK, --EBCBS
      ...
      INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'ending CIN_OHP_DAILY_MV_W_PATID');
            COMMIT;

            INSERT INTO CIN_CIGNA_DAILY_MATCHED
            --AETNA INFO
            SELECT DISTINCT CASE WHEN DAILY.FACILITY_NAME_ROLE LIKE '%NYU%' THEN 'NYU/HJD' ELSE 'OTHER' END AS NYU_OTHER,
            DAILY.PAT_MRN_ID,
            DAILY.PAT_ID,
            COALESCE(DAILY.ACTION_DATE, DAILY.DATE_ADDED) AS DATE_ADDED,
            ...
            CASE WHEN PAT.BIRTH_DATE IS NOT NULL THEN CONVERT(VARCHAR(10), PAT.BIRTH_DATE, 101) ELSE CONVERT(VARCHAR(10), CONVERT(DATE, DAILY.DATE_OF_BIRTH, 120), 101) END AS BIRTHDATE,
            ...
            NULL, --DISP_CODE_DESC,
            NULL, --RPM_SCORE,
            CONVERT(VARCHAR(10), CONVERT(DATE, DAILY.PCMH_ATTR_EFFECTIVE_DATE, 120), 101) ,
            DAILY.PULSE_SCORE,
            DAILY.AOS_SCORE, 
            DAILY.MEMBER_PHONE,
            ...
            CASE WHEN DAILY.PAT_ID NOT IN (SELECT CIN_HISTORY_DAILY_WEBI.PAT_ID FROM CIN_HISTORY_DAILY_WEBI WHERE CIN_HISTORY_DAILY_WEBI.PAYOR = 'AETNA' AND DAILY.ACTUAL_ADMIT_DATE=CIN_HISTORY_DAILY_WEBI.ADMIT_DATE AND DAILY.ACTUAL_DISCHARGE_DATE=CIN_HISTORY_DAILY_WEBI.ACTUAL_DISCHARGE_DATE AND DAILY.PAT_ID=CIN_HISTORY_DAILY_WEBI.PAT_ID AND (CIN_HISTORY_DAILY_WEBI.DATE_RUN) < CAST(GETDATE() AS DATE)) THEN 'Y' --Has not been on the webi before with this payor and admit date --cant use, havent kept history long enough yet -history has been there for a while now added 3/4/16
                 WHEN DAILY.CUMB_ID IN (SELECT CUMB_ID FROM CIN_AETNA_DAILY_PT WHERE DATE_ADDED = CAST(GETDATE() AS DATE) AND CUMB_ID NOT IN (SELECT PT1.CUMB_ID FROM CIN_AETNA_DAILY_PT PT1, CIN_AETNA_DAILY_PT PT2 WHERE PT1.DATE_ADDED = CAST(GETDATE() AS DATE) AND PT1.CUMB_ID = PT2.CUMB_ID AND PT1.BIRTH_DATE = PT2.BIRTH_DATE AND PT1.CONDITION = PT2.CONDITION AND PT1.DATE_ADDED-1 = PT2.DATE_ADDED)) THEN 'Y' --IF they qualified to make the webi today, but werent also qualified yesterday with the same condition
                 ELSE 'N'
            END AS SEND_ONBASE_DOC, 
            NULL AS AGE, --EBCBS
            NULL AS HOT_SPOTTER_CHRONIC, --EBCBS
            ...
            DAILY.ACO_ATTR_PROV_CITY
            DAILY.ACO_ATTR_PROV_STATE,
            DAILY.ACO_ATTR_PROV_ZIPCODE,
            NULL AS PROVIDER_SPECIALTY,--EBCBS
            NULL AS ER_VISITS,--EBCBS
            NULL AS NO_OF_REFERRALS_RCVD,--EBCBS
            NULL AS NO_OF_REFERRALS_SENT,--EBCBS
      ...
      INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'ending CIN_AETNA_DAILY_MV_W_PATID');
            COMMIT;

            INSERT INTO CIN_CIGNA_DAILY_MATCHED
            --EBCBS INFO
            SELECT DISTINCT CASE WHEN DAILY.INPATIENT_FACILITY LIKE '%NYU%' THEN 'NYU/HJD' ELSE 'OTHER' END AS NYU_OTHER,
            DAILY.PAT_MRN_ID,
            DAILY.PAT_ID,
            COALESCE(DAILY.ACTION_DATE, DAILY.DATE_ADDED) AS DATE_ADDED,
            UPPER(CASE WHEN PAT.PAT_LAST_NAME IS NOT NULL THEN PAT.PAT_LAST_NAME ELSE DAILY.PATIENT_LAST_NAME END),
            UPPER(CASE WHEN PAT.PAT_FIRST_NAME IS NOT NULL THEN PAT.PAT_FIRST_NAME ELSE DAILY.PATIENT_FIRST_NAME END),
            DAILY.GENDER,
            CASE WHEN PAT.BIRTH_DATE IS NOT NULL THEN CONVERT(VARCHAR(10), PAT.BIRTH_DATE, 101) ELSE DAILY.DOB END,
            ...
            CASE WHEN DAILY.PAT_ID NOT IN (SELECT CIN_HISTORY_DAILY_WEBI.PAT_ID FROM CIN_HISTORY_DAILY_WEBI WHERE CIN_HISTORY_DAILY_WEBI.PAYOR = DAILY.PAYER AND DAILY.ADMISSION_DATE=CIN_HISTORY_DAILY_WEBI.ADMIT_DATE AND DAILY.DISCHARGE_DATE=CIN_HISTORY_DAILY_WEBI.ACTUAL_DISCHARGE_DATE AND DAILY.PAT_ID=CIN_HISTORY_DAILY_WEBI.PAT_ID AND (CIN_HISTORY_DAILY_WEBI.DATE_RUN) < CAST(GETDATE() AS DATE)) THEN 'Y' --Has not been on the webi before with this payor and admit date --cant use, havent kept history long enough yet -history has been there for a while now added 3/4/16
                 WHEN DAILY.MEMBER_ID IN (SELECT MEMBER_ID FROM CIN_EBCBS_DAILY_PT WHERE DATE_ADDED = CAST(GETDATE() AS DATE) AND MEMBER_ID NOT IN (SELECT PT1.MEMBER_ID FROM CIN_EBCBS_DAILY_PT PT1, CIN_EBCBS_DAILY_PT PT2 WHERE PT1.DATE_ADDED = CAST(GETDATE() AS DATE) AND PT1.MEMBER_ID = PT2.MEMBER_ID AND PT1.CONDITION = PT2.CONDITION AND PT1.DATE_ADDED-1 = PT2.DATE_ADDED)) THEN 'Y'  --IF they qualified to make the webi today, but werent also qualified yesterday with the same condition
                 ELSE 'N'
            END AS SEND_ONBASE_DOC,
            DAILY.AGE, --EBCBS
      ...
      INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'ending CIN_EBCBS_DAILY_MV_W_PATID');
            COMMIT;

            INSERT INTO CIN_CIGNA_DAILY_MATCHED
            --AETNAMA INFO added 6/20/2017
            SELECT DISTINCT CASE WHEN DAILY.FACILITY_NAME LIKE '%NYU%' THEN 'NYU/HJD' ELSE 'OTHER' END AS NYU_OTHER,
            DAILY.PAT_MRN_ID,
            DAILY.PAT_ID,
            COALESCE(DAILY.ACTION_DATE, DAILY.DATE_ADDED) AS DATE_ADDED,
            ...
            CASE WHEN DAILY.GENDER IS
 NOT NULL THEN DAILY.GENDER WHEN PAT.SEX_C = 1 THEN 'F' WHEN PAT.SEX_C = 2 THEN 'M' ELSE 'Unknown' END AS GENDER, --TABLE WAS BLANK WITH GENDER SO PULLING FROM EPIC IF NULL FROM HEALTHPLS TABLE gl 7/6/17
            --CASE WHEN PAT.BIRTH_DATE IS NOT NULL THEN CONVERT(VARCHAR(10), PAT.BIRTH_DATE, 101) ELSE DAILY.DOB END AS DOB,  source date format changed
            CASE WHEN PAT.BIRTH_DATE IS NOT NULL THEN CONVERT(VARCHAR(10), PAT.BIRTH_DATE, 101) ELSE CONVERT(VARCHAR(10), CONVERT(DATE, DAILY.DOB, 120), 101) END AS DOB,
            DAILY.MEMBER_ID, --CIGNA ID FIELD
            ...
            DAILY.PAYER AS PAYOR,
            CASE WHEN DAILY.PAT_ID NOT IN (SELECT CIN_HISTORY_DAILY_WEBI.PAT_ID FROM CIN_HISTORY_DAILY_WEBI WHERE CIN_HISTORY_DAILY_WEBI.PAYOR = DAILY.PAYER AND DAILY.ADMISSION_DATE=CIN_HISTORY_DAILY_WEBI.ADMIT_DATE AND DAILY.DISCHARGE_DATE=CIN_HISTORY_DAILY_WEBI.ACTUAL_DISCHARGE_DATE AND DAILY.PAT_ID=CIN_HISTORY_DAILY_WEBI.PAT_ID AND (CIN_HISTORY_DAILY_WEBI.DATE_RUN) < CAST(GETDATE() AS DATE)) THEN 'Y' --Has not been on the webi before with this payor and admit date --cant use, havent kept history long enough yet -history has been there for a while now added 3/4/16
                 WHEN DAILY.MEMBER_ID IN (SELECT MEMBER_ID FROM CIN_AETNAMA_DAILY_PT WHERE DATE_ADDED = CAST(GETDATE() AS DATE) AND MEMBER_ID NOT IN (SELECT PT1.MEMBER_ID FROM CIN_AETNAMA_DAILY_PT PT1, CIN_AETNAMA_DAILY_PT PT2 WHERE PT1.DATE_ADDED = CAST(GETDATE() AS DATE) AND PT1.MEMBER_ID = PT2.MEMBER_ID AND PT1.CONDITION = PT2.CONDITION AND PT1.DATE_ADDED-1 = PT2.DATE_ADDED)) THEN 'Y'  --IF they qualified to make the webi today, but werent also qualified yesterday with the same condition
                 ELSE 'N' 
            END AS SEND_ONBASE_DOC,
            NULL AS AGE, --EBCBS
            NULL AS HOT_SPOTTER_CHRONIC, --EBCBS
      ...
      INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'ending CIN_AETNAMA_DAILY_MV_W_PATID');
            COMMIT;
            ...
            DAILY.PAYER AS PAYOR,
            CASE WHEN DAILY.PAT_ID NOT IN (SELECT CIN_HISTORY_DAILY_WEBI.PAT_ID FROM CIN_HISTORY_DAILY_WEBI WHERE CIN_HISTORY_DAILY_WEBI.PAYOR = DAILY.PAYER AND DAILY.ADMISSION_DATE=CIN_HISTORY_DAILY_WEBI.ADMIT_DATE AND DAILY.DISCHARGE_DATE=CIN_HISTORY_DAILY_WEBI.ACTUAL_DISCHARGE_DATE AND DAILY.PAT_ID=CIN_HISTORY_DAILY_WEBI.PAT_ID AND (CIN_HISTORY_DAILY_WEBI.DATE_RUN) < CAST(GETDATE() AS DATE)) THEN 'Y' --Has not been on the webi before with this payor and admit date --cant use, havent kept history long enough yet -history has been there for a while now added 3/4/16
                 WHEN DAILY.MEMBER_ID IN (SELECT MEMBER_ID FROM CIN_HEALTHPMC_DAILY_PT WHERE DATE_ADDED = CAST(GETDATE() AS DATE) AND MEMBER_ID NOT IN (SELECT PT1.MEMBER_ID FROM CIN_HEALTHPMC_DAILY_PT PT1, CIN_HEALTHPMC_DAILY_PT PT2 WHERE PT1.DATE_ADDED = CAST(GETDATE() AS DATE) AND PT1.MEMBER_ID = PT2.MEMBER_ID AND PT1.CONDITION = PT2.CONDITION AND PT1.DATE_ADDED-1 = PT2.DATE_ADDED)) THEN 'Y'  --IF they qualified to make the webi today, but werent also qualified yesterday with the same condition
                 ELSE 'N'
            END AS SEND_ONBASE_DOC,
            DAILY.AGE, --EBCBS
            DAILY.HOT_SPOTTER_CHRONIC, --EBCBS
      ...
      INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'ending CIN_HEALTHPMC_DAILY_MV_WPATI');
            COMMIT;

            INSERT INTO CIN_CIGNA_DAILY_MATCHED
            --THIS IS TO PULL PATIENTS THAT WERE ADDED FROM THE ATTRIBUTION FILE METHOD
            SELECT DISTINCT 'NYU/HJD' AS NYU_OTHER,
            DAILY.PAT_MRN_ID,
            DAILY.PAT_ID,
            ...
            CASE WHEN PAT.SEX_C = 1 THEN 'F' WHEN PAT.SEX_C = 2 THEN 'M' ELSE 'Unknown' END AS GENDER, --GENDER,
            CONVERT(VARCHAR(10), DAILY.BIRTH_DATE, 101),
            ...
            NULL, --CIGNA BEHAVIORAL
            NULL, --DAILY.EMP_NAME, --ACCOUNT NAME
            CONVERT(VARCHAR(10), DAILY.HOSP_ADMSN_TIME, 101), --DAILY.ADMITDATE, 
            NULL, --NEW ENTRY ON REPORT
            NULL, --PROJECTED DISCHARGE DATE
            CONVERT(VARCHAR(10), DAILY.HOSP_DISCH_TIME, 101), --DAILY.DISCHARGEDATE,
            CONVERT(VARCHAR(10), DAILY.LOS), --DAILY.LOS,
            ...
            CONVERT(VARCHAR(10), DAILY.AGE_AT_ADMISSION) AS AGE, --EBCBS
            NULL AS HOT_SPOTTER_CHRONIC, --EBCBS
            NULL AS HOT_SPOTTER_CHRONIC_MONTHS, --EBCBS
            NULL AS HOT_SPOTTER_READMISSION, --EBCBS
            NULL AS RISK_DRIVERS, --EBCBS
      ...
      INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'ending REPORTADMIN.CIN_ATTR_DAILY_PT');
            COMMIT;


      INSERT INTO job_status VALUES(NEXT VALUE FOR job_seq,'SP_CIN_CIGNA_DAILY_MATCHED', GETDATE(), 'ending refresh');
      COMMIT;
